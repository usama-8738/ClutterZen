...
class FakeAnalysisRepository implements IAnalysisRepository {
  ...
  Future<void> saveAnalysis(
    ...
    list.insert(
      ...
      StoredAnalysis(
        ...
        imageUrl: imageUrl,
        title: analysis.labels.isEmpty
            ? 'Scan'
            : analysis.labels.take(2).join(' '),
        clutterScore: (analysis.objects.length.clamp(0, 50) / 5)
            .toDouble()
            .clamp(1.0, 10.0),
        primaryCategory: analysis.labels.isNotEmpty
            ? analysis.labels.first.toLowerCase()
            : 'general',
        categories:
            analysis.labels.take(5).map((e) => e.toLowerCase()).toList(),
  ...
  Stream<List<StoredAnalysis>> watchUserAnalyses(String uid, {int limit = 20}) {
    ...
    void emit() =>
        controller.add(List<StoredAnalysis>.from(_userData[uid] ?? const []));
    emit();
