    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          Row(
            children: [
              CircleAvatar(radius: 30, child: Text(_initials(user?.displayName))),
              const SizedBox(width: 12),
              Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text(user?.displayName ?? 'Guest', style: Theme.of(context).textTheme.titleMedium?.copyWith(fontWeight: FontWeight.bold)),
                Text(user?.email ?? '', style: Theme.of(context).textTheme.bodySmall?.copyWith(color: Colors.grey[600])),
              ]),
            ],
          ),
          ...
          Container(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Row(children: [
                const Text('Free Plan', style: TextStyle(fontWeight: FontWeight.w700)),
                const SizedBox(width: 8),
                Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(12)), child: const Text('Upgrade')),
              ]),
              const SizedBox(height: 8),
              Row(children: const [Icon(Icons.camera_alt_outlined, size: 18), SizedBox(width: 6), Text('2/3 scans used this month')]),
          ...
          ListTile(
            ...
            onTap: () async { await FirebaseAuth.instance.signOut(); if (context.mounted) Navigator.of(context).pushNamedAndRemoveUntil('/splash', (_) => false); },
          ),
        ],
      ),
    );
