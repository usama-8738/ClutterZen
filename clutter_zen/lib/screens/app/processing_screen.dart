...
class _ProcessingScreenState extends State<ProcessingScreen> {
  static const _lottieUrl = 'https://lottie.host/0bd5139f-6801-4bfe-abdf-e4e03d90ab03/2DCtc5jJKu.json';
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [Row(children: const [Icon(Icons.camera_alt_outlined), SizedBox(width: 4), Text('3'), SizedBox(width: 12)])],
      ),
      body: Stack(
        children: [
          Positioned.fill(
          ...
          Center(
            child: Container(
              ...
              child: Column(
                ...
                children: [
                  SizedBox(width: 150, height: 150, child: Lottie.network(_lottieUrl, repeat: true)),
                  const SizedBox(height: 16),
                  Text('Analyzing Your Image', style: Theme.of(context).textTheme.titleLarge, textAlign: TextAlign.center),
                  const SizedBox(height: 12),
                  // Steps
                  for (int i = 0; i < _steps.length; i++) _ProcessingStep(text: _steps[i], isActive: _currentStep == i, isComplete: _currentStep > i),
                  const SizedBox(height: 16),
                  Text(_tips[_tipIndex], style: Theme.of(context).textTheme.bodySmall?.copyWith(color: Colors.grey[600]), textAlign: TextAlign.center),
                ],
              ),
            ),
          ),
...
class _ProcessingStep extends StatelessWidget {
  const _ProcessingStep({required this.text, required this.isActive, required this.isComplete});
  final String text;
  final bool isActive;
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      ...
      child: Row(
        mainAxisSize: MainAxisSize.min,
        children: [icon, const SizedBox(width: 8), Flexible(child: Text(text, style: style))],
      ),
    );
  }
}
